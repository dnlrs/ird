...
def inizializzazione(fileInput, geneNames):
	"""
	Inizializzazione delle strutture dati per il progetto B.

	Tali struttura sono necessarie all'elaborazione degli introni, alla
	mappatura delle reads e alla produzione del file di output.
...
def sortRegioni(tupla):
	"""Funzione utilizzata per ordinare 3 liste rispetto alla prima
...
def caricaReadsEsIn(fileInput):
	"""
	Carica in memoria il file '.gtf' con tutti gli Esoni e gli Introni.

	Argomenti:
		fileInput		:		nome del file '.gtf' totale

	Ritorna:
		dictReadsEsIn	:		dizionario con gene e per ogni gene un dizionario
								di cromosomi con gli intervalli mappati
	"""
...
def stampaBedGraph(dictReadsEsIn, dictGeneChr, fileOut, geneNames):
	"""
	Stampa i file '.bedGraph' per ogni gene, differenziando sui cromosomi,
	con reads mappanti.
